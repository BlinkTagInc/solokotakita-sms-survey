...
module.exports = {
  ...
  doSurvey: function(app, req, res) {
    ...
    Survey.findOne({src: req.param('src')}, function(e, survey){
      ...
      survey.save(function(e){
        sendMessage(req, res, nextQuestion, survey.src, surveyStatus);
      });
    });
  },
  ...
  resetQuestion: function(app, req, res) {
    ...
    Survey.findOne({ src: req.param('src') }, function(e, survey) {
      if (survey) {
        ...
        resetMessage = "Nothing to reset.  Text 'Mulai' to start the survey.";
      }
      //Send notice that survey has been reset
      sendMessage(req, res, resetMessage, req.param('src'), (survey.answers.length + 1));
    });
  },
  ...
  resetSurvey: function(app, req, res) {
    ...
    Survey.remove({ src: req.param('src') }, function(e) {
      sendMessage(req, res, resetMessage, req.param('src'), 1);
    });
  },
...
function sendMessage(req, res, msg, dst, status) {
